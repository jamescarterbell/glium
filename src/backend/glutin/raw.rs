...
pub struct GlutinBackend{
    context: Rc<RefCell<Takeable<glutin::ContextWrapper<Pc, ()>>>>,
    size: (u32, u32),
}

/// Error that can happen while creating a glium display.
#[derive(Debug)]
pub enum RawCreationError {
...
impl Raw {
    /// Create a new glium `Raw`.
    ///
    /// Performs a compatibility check to make sure that all core elements of glium are supported
    /// by the implementation.
    pub fn from_gl_window<T: ContextCurrentState>(gl_context: glutin::RawContext<T>) -> Result<Self, IncompatibleOpenGl> {
        Self::with_debug(gl_context, Default::default())
    }
    ...
    pub unsafe fn unchecked<T: ContextCurrentState>(gl_context: glutin::RawContext<T>) -> Result<Self, IncompatibleOpenGl> {
        Self::unchecked_with_debug(gl_context, Default::default())
    }

    /// The same as the `new` constructor, but allows for specifying debug callback behaviour.
    pub fn with_debug<T: ContextCurrentState>(gl_context: glutin::RawContext<T>, debug: debug::DebugCallbackBehavior)
        -> Result<Self, IncompatibleOpenGl>
    {
        Self::new_inner(gl_context, debug, true)
    }
    ...
    pub unsafe fn unchecked_with_debug<T: ContextCurrentState>(
        gl_context: glutin::RawContext<T>,
        debug: debug::DebugCallbackBehavior,
    ) -> Result<Self, IncompatibleOpenGl>
    {
        Self::new_inner(gl_context, debug, false)
    }
    ...
    ) -> Result<Self, IncompatibleOpenGl>
    {
        let gl_context = unsafe {
            gl_context.treat_as_current()
        };
        let gl_context = Rc::new(RefCell::new(Takeable::new(gl_context)));
        ...
        Ok(Raw {
            gl_context: glutin_backend,
            context: context,
            last_framebuffer_dimensions: Cell::new(framebuffer_dimensions),
        })
    }

    /// Borrow the inner glutin RawContext.
    #[inline]
    pub fn gl_context(&self) -> Ref<Takeable<glutin::RawContext<Pc>>> {
    ...
    pub fn draw(&self) -> Frame {
        ...
        if self.last_framebuffer_dimensions.get() != (w, h) {
            ...
            self.gl_context.borrow().resize((w, h).into());
        }
...
impl fmt::Display for RawCreationError {
    fn fmt(&self, fmt: &mut fmt::Formatter) -> Result<(), fmt::Error> {
        match self {
            RawCreationError::GlutinCreationError(err) => write!(fmt, "{}", err),
            RawCreationError::IncompatibleOpenGl(err) => write!(fmt, "{}", err),
        }
    }
}

impl Error for RawCreationError {
    #[inline]
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match *self {
            RawCreationError::GlutinCreationError(ref err) => Some(err),
            RawCreationError::IncompatibleOpenGl(ref err) => Some(err),
        }
    }
}

impl From<glutin::CreationError> for RawCreationError {
    #[inline]
    fn from(err: glutin::CreationError) -> RawCreationError {
        RawCreationError::GlutinCreationError(err)
    }
}

impl From<IncompatibleOpenGl> for RawCreationError {
    #[inline]
    fn from(err: IncompatibleOpenGl) -> RawCreationError {
        RawCreationError::IncompatibleOpenGl(err)
    }
}
...
impl Deref for GlutinBackend {
    type Target = Rc<RefCell<Takeable<glutin::RawContext<Pc>>>>;
    #[inline]
    fn deref(&self) -> &Rc<RefCell<Takeable<glutin::RawContext<Pc>>>> {
        &self.context
    }
}
...
unsafe impl Backend for GlutinBackend {
    ...
    unsafe fn make_current(&self) {
        ...
        let gl_context = Takeable::take(&mut gl_window_takeable);
        let new_gl_window = gl_context.make_current().unwrap();
